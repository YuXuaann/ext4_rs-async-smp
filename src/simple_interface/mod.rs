...
impl Ext4 {
    ...
    pub async fn ext4_file_open(&self, path: &str, flags: &str) -> Result<u32> {
    ...
    pub async fn ext4_dir_mk(&self, path: &str) -> Result<u32> {
        let mut search_result = Ext4DirSearchResult::new(Ext4DirEntry::default());
        let r = self
            .dir_find_entry(ROOT_INODE, path, &mut search_result)
            .await;
        if r.is_ok() {
    ...
    pub async fn ext4_dir_open(&self, path: &str) -> Result<u32> {
        let mut parent_inode_num = ROOT_INODE;
        let filetype = InodeFileType::S_IFDIR;
    ...
    pub async fn ext4_file_read(&self, ino: u64, size: u32, offset: i64) -> Result<Vec<u8>> {
    ...
    pub async fn ext4_file_write(&self, ino: u64, offset: i64, data: &[u8]) -> Result<usize> {
